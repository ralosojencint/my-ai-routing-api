...
async def research(query):
    ...
    def valid(items):
        out=[]
        for x in items:
            if not is_relevant_ai_source(x,today,target_date=target_date): continue
            if exact_today or target_date==today:
                published = source_date(x)
                # Explicitly old/mismatched dates are rejected; missing metadata
                # remains eligible because the Tavily search is day-scoped.
    ...
    if len(filtered)<requested_count and (exact_today or target_date==today):
        recovery=[
            ...
            f"AI technology today {today.strftime('%B %d %Y')}",
            f"AI product launch today {today.strftime('%B %d %Y')}",
            f"AI research today {today.strftime('%B %d %Y')}",
            f"AI robotics today {today.strftime('%B %d %Y')}",
        ]
        more=await asyncio.gather(*(search_one(q,"day") for q in recovery))
        all_sources.extend(x for b in more for x in b)
        filtered=valid(unique(all_sources))
