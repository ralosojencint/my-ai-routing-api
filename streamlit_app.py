...
st.markdown("""
    ...
    div.stTextInput > div > div > input { background-color: #1e202a !important; color: white !important; border-radius: 25px !important; border: 1px solid #2e3244 !important; padding-left: 20px !important; height: 48px !important; font-size: 14px !important; }
    ...
    div[data-testid="stFileUploaderDropzone"] button { background-color: #2e3244 !important; color: white !important; border-radius: 20px !important; height: 40px !important; font-size: 14px !important; font-weight: bold !important; }
    .send-btn-box button { background-color: #d0755d !important; color: white !important; border-radius: 50% !important; height: 44px !important; width: 44px !important; min-width: 44px !important; border: none !important; font-size: 18px !important; font-weight: bold !important; display: flex !important; align-items: center !important; justify-content: center !important; padding: 0 !important; }
...
if not st.session_state["is_premium"] and st.session_state["anonymous_clicks"] >= FREE_DAILY_LIMIT:
    ...
    if st.session_state["text_out"]: output_holder.markdown(f"### 📊 Live System Engine Outputs\n{st.session_state['text_out']}")
    ...
    user_input = st.text_input("", placeholder="Ask anything, paste links, or generate AI art...", label_visibility="collapsed")
    
    col_left, col_mid, col_right = st.columns([1.5, 1.5, 1])
    with col_left: uploaded_image = st.file_uploader("+ Attach File", type=["png", "jpg", "jpeg"], label_visibility="collapsed")
    with col_mid: audio_file = st.audio_input("🎤 Record Voice", label_visibility="collapsed")
    with col_right:
    ...
    if execute_btn:
        if not user_input and not uploaded_image and not audio_file:
            ...
            if generate_art_mode:
                ...
                if len(numbers) >= 2: st.session_state["text_out"] = f"```text\n💡 Programmatic Compute:\n{numbers} + {numbers} = {numbers + numbers}\n```"
