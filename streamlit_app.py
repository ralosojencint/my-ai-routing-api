...
async def research(query):
    ...
    def unique(items):
        ...
        return out
    # Date integrity: prefer today-only evidence. Only fall back to yesterday
    # when today cannot satisfy the requested count, and record that fallback.
    allowed_today={today}
    allowed_dates=allowed_today
    def valid(items, allowed):
        out=[]
        for item in items:
            ...
            if (exact_today or target_date==today):
                if published is None or published not in allowed: continue
            elif target_date is not None and published is not None and published!=target_date: continue
            # Final research sources must contain substantive evidence, not just a headline.
            title=clean_text(item.get("title","")); content=clean_text(item.get("content",""))
            if len(title)<12 or len(content)<40: continue
            out.append(item)
        return out
    unique_sources=unique(all_sources); filtered=valid(unique_sources, allowed_today)

    # Phase-3 date fallback: yesterday is allowed only when today's evidence is
    # insufficient. Never mix dates when three same-day sources are available.
    used_previous_day=False
    if len(filtered)<requested_count and day_scoped:
        previous_day=today.fromordinal(today.toordinal()-1)
        previous=valid(unique_sources,{previous_day})
        if previous:
            filtered.extend(previous)
            used_previous_day=True
            st.session_state.activity.append(
                f"Research date fallback: today yielded {len(filtered)-len(previous)} qualifying source(s); added {previous_day.isoformat()} evidence"
            )
    ...
    if len(filtered)<requested_count and day_scoped:
        ...
        if supplemental_sources:
            ...
            filtered=valid(unique_sources,allowed_today)
    ...
    if len(filtered)<requested_count and (exact_today or target_date==today):
        # No date relaxation beyond today/yesterday; title-only or stale items are rejected.
        pass
...
def source_grounded_summary(source):
    """Return a substantive sentence from the retrieved source, never title-only text."""
    title = clean_text(source.get("title", ""))
    content = clean_text(source.get("content", ""))
    ...
    for sentence in sentences:
        ...
        score=overlap*3+event_bonus-max(0,len(sentence)-420)/80
        candidates.append((score,sentence))
    snippet=max(candidates,key=lambda x:x[0])[1] if candidates else content[:420].strip()
    if len(snippet)>500: snippet=snippet[:497].rsplit(" ",1)[0]+"…"
    elif snippet and snippet[-1] not in ".!?…": snippet+="…"
    return snippet


def source_organizations(source):
    """Extract organization names conservatively from title/content text."""
    title=clean_text(source.get("title","")); content=clean_text(source.get("content",""))
    text=f"{title}. {content[:1200]}"
    known=r"\b(?:OpenAI|Anthropic|Google(?: DeepMind)?|Microsoft|Meta|NVIDIA|Amazon|Apple|xAI|Mistral|DeepSeek|Alibaba|Baidu|ByteDance|Samsung|SK Telecom|Faraday Future|Andreessen Horowitz|a16z|Endless Surf|Flowstate|Voiceware AI|Agentrixapp)\b"
    found=[]
    for name in re.findall(known,text,re.I):
        canonical=name.strip()
        if canonical.lower() not in {x.lower() for x in found}: found.append(canonical)
    # Also capture capitalized organization-like phrases from the headline when
    # they are not in the known list. Keep this conservative to avoid inventing names.
    for chunk in re.findall(r"\b[A-Z][A-Za-z0-9&.-]*(?:\s+[A-Z][A-Za-z0-9&.-]*){0,3}",title):
        low=chunk.lower().strip()
        if low in {"what happened","why it matters","publication date"}: continue
        if any(word in low.split() for word in ("launches","launch","announces","announced","new","company","standard","exclusive")): continue
        if len(chunk)>=3 and chunk.lower() not in {x.lower() for x in found}: found.append(chunk.strip(" -–—:"))
    return ", ".join(found[:5])


def source_significance(source):
    """Give a narrowly source-grounded significance statement; never invent market impact."""
    content=clean_text(source.get("content","")); title=clean_text(source.get("title",""))
    text=f"{title}. {content}".strip()
    if not text: return ""
    low=text.lower()
    signals=("launch", "launches", "announc", "release", "standard", "partnership", "fund", "investment", "deployment", "factory", "infrastructure", "research")
    if any(x in low for x in signals):
        summary=source_grounded_summary(source)
        if summary:
            return "The source presents this as a material AI launch, announcement, partnership, investment, infrastructure, or research development."
    return "The source does not independently establish broader significance."
...
def validate_research_output(draft, query, sources):
    """Strict Phase-3 contract: structure, source isolation, dates, and source list."""
    text=clean_ai_response(draft)
    if not text or not sources: return False
    requested=requested_development_count(query,default=len(sources)); verified=min(requested,len(sources))
    headings=re.findall(r"(?im)^\s*#{2,6}\s+Sources\s*$",text)
    if len(headings)!=1: return False
    m=re.search(r"(?im)^\s*#{2,6}\s+Sources\s*$",text)
    body=text[:m.start()]; sources_section=text[m.end():]
    blocks=[]
    for match in re.finditer(r"(?ms)^\s*(\d+)\.\s+(.*?)(?=^\s*\d+\.\s+|\Z)",body):
        blocks.append((int(match.group(1)),match.group(0)))
    if [n for n,_ in blocks] != list(range(1,verified+1)): return False
    if len(blocks)!=verified: return False
    for index,block in blocks:
        low=block.lower()
        if not all(x in low for x in ("what happened:","organizations involved:","why it matters:","publication date:")): return False
        ids={int(x) for x in re.findall(r"\[Source\s+(\d+)\]",block,re.I)}
        if ids!={index}: return False
        if re.search(r"\bpublication date:\s*not provided",low): return False
        expected=source_date(sources[index-1])
        if expected and expected.isoformat() not in block: return False
    listed=[int(x) for x in re.findall(r"(?m)^\s*(\d+)\.\s+",sources_section)]
    if listed != list(range(1,verified+1)): return False
    cited=[]
    for _,block in blocks:
        cited.extend(int(x) for x in re.findall(r"\[Source\s+(\d+)\]",block,re.I))
    cited_unique=[]
    for x in cited:
        if x not in cited_unique: cited_unique.append(x)
    if cited_unique != list(range(1,verified+1)): return False
    return True


def render_exact_research_output(query,sources):
    """Deterministic Phase-3 fallback using only retrieved source evidence."""
    n=requested_development_count(query,default=len(sources)); selected=sources[:n]
    if not selected: return ""
    items=[]; source_lines=[]
    if len(selected)<n:
        items.append(f"Only {len(selected)} independently verified development(s) were available; {n} requested. NEXUS will not invent or duplicate a development.")
    for i,src in enumerate(selected,1):
        title=clean_text(src.get("title","")); summary=source_grounded_summary(src)
        url=str(src.get("url","" )).strip(); published=source_date(src)
        if not title or not summary or not url: return ""
        orgs=source_organizations(src) or "The source does not provide enough evidence to identify additional organizations."
        significance=source_significance(src)
        published_text=published.isoformat() if published else "not provided by the source"
        items.append(
            f"{i}. **{title}**\n"
            f"- What happened: {summary} [Source {i}]\n"
            f"- Organizations involved: {orgs} [Source {i}]\n"
