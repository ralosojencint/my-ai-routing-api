...
async def research(query):
    ...
    # For a "today" request, Tavily's day scope is the date filter. Do not
    # require the literal ISO date to appear in indexed article text.
    if exact_today or target_date == today:
        queries=[
            focus,
            "AI artificial intelligence news developments",
            "AI model launch release announcement",
            "OpenAI Anthropic Google Gemini Meta AI NVIDIA announcement",
            "AI research study breakthrough",
            "AI agents robotics product deployment",
            "AI regulation policy government",
            "AI chips infrastructure technology announcement",
            "generative AI enterprise deployment product",
        ]
    else:
        queries=[
            f"{focus} {date_text}",
            f"AI artificial intelligence news developments {date_text}",
            f"AI model launch release announcement {date_text}",
            f"OpenAI Anthropic Google Gemini Meta AI NVIDIA announcement {date_text}",
            f"AI research study breakthrough {date_text}",
            f"AI agents robotics product deployment {date_text}",
            f"AI regulation policy government {date_text}",
            f"AI chips infrastructure technology announcement {date_text}",
            f"generative AI enterprise deployment product {date_text}",
        ]
    async def search_one(q,time_range="day"):
        kwargs={"query":q,"search_depth":"advanced","topic":"news","max_results":10,"include_answer":False}
        if time_range: kwargs["time_range"]=time_range
        try:
            r=await asyncio.to_thread(client.search,**kwargs)
            return [x for x in (r.get("results",[]) if isinstance(r,dict) else []) if isinstance(x,dict)]
        except Exception as exc:
            # A failed news search should not silently become a false zero.
            # Retry once without the news topic while preserving day scope.
            try:
                fallback={"query":q,"search_depth":"advanced","max_results":10,"include_answer":False}
                if time_range: fallback["time_range"]=time_range
                r=await asyncio.to_thread(client.search,**fallback)
                return [x for x in (r.get("results",[]) if isinstance(r,dict) else []) if isinstance(x,dict)]
            except Exception:
                st.session_state.activity.append(f"Research search failed: {type(exc).__name__}")
                return []
    batches=await asyncio.gather(*(search_one(q,"day" if (exact_today or target_date==today) else None) for q in queries))
    ...
    def unique(items):
        ...
        return out
    # Tavily/news providers can straddle UTC and Asia/Manila around midnight.
    # For an explicit "today" request, dated evidence from today OR the
    # immediately previous calendar day is eligible; anything older is rejected.
    allowed_dates = {today}
    if exact_today or target_date == today:
        allowed_dates.add(today.fromordinal(today.toordinal() - 1))

    def valid(items):
        out=[]
        for x in items:
            if not is_relevant_ai_source(x,today,target_date=None if (exact_today or target_date==today) else target_date):
                continue
            published = source_date(x)
            if exact_today or target_date == today:
                # Missing metadata remains eligible because the search itself is
                # day-scoped. Explicitly older dates do not.
                if published is not None and published not in allowed_dates:
                    continue
            elif target_date is not None and published is not None and published != target_date:
                continue
            out.append(x)
        return out
    ...
    # Second-stage recovery: preserve the same-day publication requirement but
    # relax only the event classifier. This prevents valid AI news from being
    # discarded because a headline uses an unexpected event verb.
    if len(filtered)<requested_count and (exact_today or target_date==today):
        ...
        # Keep strict candidates first, then use relaxed candidates to fill the gap.
        merged=filtered[:]
        for source in sorted(relaxed,key=lambda x:research_score(x,today),reverse=True):
            if normalize_url(source.get("url","")) in {normalize_url(x.get("url","")) for x in merged}:
                continue
    ...
    if len(filtered)<requested_count and (exact_today or target_date==today):
        recovery=[
            f"AI news {today.strftime('%B %d %Y')}",
            f"AI news {(today.fromordinal(today.toordinal() - 1)).strftime('%B %d %Y')}",
            f"artificial intelligence announcement {today.strftime('%B %d %Y')}",
            f"artificial intelligence announcement {(today.fromordinal(today.toordinal() - 1)).strftime('%B %d %Y')}",
            f"AI product launch {today.strftime('%B %d %Y')}",
            f"AI product launch {(today.fromordinal(today.toordinal() - 1)).strftime('%B %d %Y')}",
            f"AI research breakthrough {today.strftime('%B %d %Y')}",
            f"AI agents robotics deployment {today.strftime('%B %d %Y')}",
        ]
        # Keep the date constraint in the query, but do not rely solely on the
        # provider's day filter. This catches UTC/provider boundary cases while
        # the validation gate still rejects genuinely old evidence.
        more=await asyncio.gather(*(search_one(q,None) for q in recovery))
        all_sources.extend(x for b in more for x in b)
        candidate_pool=unique(all_sources)
        recovered=valid(candidate_pool)
        # Preserve everything already recovered; never rebuild from a strict
        # subset and accidentally throw away relaxed candidates.
        merged=filtered[:]
        existing_urls={normalize_url(x.get("url","")) for x in merged}
    ...
    # Compact diagnostic required for live Phase-2 debugging.
    st.session_state.activity.append(
        f"Research diagnostic: requested={requested_count} raw={len(all_sources)} "
        f"unique={len(unique(all_sources))} verified={len(filtered)} selected={len(final)}"
    )
