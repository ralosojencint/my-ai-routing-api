...
def is_relevant_ai_source(source, today):
    ...
    # Tavily's news/time_range filter is already scoped to today. If it
    # supplies a date, enforce it; if it omits the date, do not discard an
    # otherwise valid current result solely because metadata is missing.
    if published is not None and published != today:
        return False

    title_lower = title.lower()

    # Reject prediction-market, shopping, SEO, and commentary pages even
    # when they happen to mention a real AI release in their article text.
    hard_noise = {
        "prediction market", "prediction markets", "release date odds",
        "release date predictions", "odds", "kalshi", "polymarket",
        "buy", "coupon", "horoscope", "weather", "recipe",
        "conference registration", "webinar", "opinion", "editorial",
        "commentary", "forecast", "price target", "stock analysis",
    }
    ...
    # For AI-news queries, AI must be central to the story. Do not accept
    # an unrelated health/science/business article merely because its body
    # mentions "AI-powered" once.
    ai_title_terms = set(RESEARCH_AI_TERMS) | {
        "gpt", "chatgpt", "claude", "llama", "copilot",
        "ai-powered", "ai-powered", "artificial intelligence",
    }
    ai_in_title = any(term in title_lower for term in ai_title_terms)

    # Major AI companies/models are valid even when the headline does not
    # literally contain the letters "AI".
    major_ai_names = {
    ...
    # The story must contain a concrete development signal. Require that
    # signal in the headline or near the beginning of the article so a
    # stray "reports/study" mention deep in an unrelated story cannot pass.
    event_window = f"{title_lower} {content[:1800].lower()}"
    if not any(term in event_window for term in EVENT_TERMS):
        return False

    # Reject obvious non-development pieces when the title is framed as
    # general commentary rather than something that happened.
    generic_commentary = {
        "what is", "how to", "guide to", "best ", "top ",
        "explained", "everything you need to know",
    }
    if any(term in title_lower for term in generic_commentary):
        return False
