...
def source_date(source):
    """Extract a publication date from provider metadata or article text.

    Tavily sometimes omits/normalizes publication metadata.  For strict
    same-day research, use metadata first, then publication-like dates in the
    title/opening article text.  A month/day without a year inherits the
    current research year.  We intentionally inspect only the title and the
    opening portion of content so an old date buried later in an article does
    not become the publication date.
    """
    ...
    # If provider metadata is absent, use the publication-like date exposed by
    # the title/opening article text.  raw_content is supported when Tavily
    # returns it instead of content.
    text_values = [
        clean_text(source.get("title", "")),
        clean_text(source.get("content", ""))[:1800],
        clean_text(source.get("raw_content", ""))[:1800],
    ]
    values.extend(v for v in text_values if v)
    ...
    for raw in values:
        # ISO provider timestamps such as 2026-08-20T14:00:00Z.
        try:
        ...
        # ISO and compact provider dates.
        ...
        # Full month-name dates: August 20, 2026 / 20 August 2026.
        ...
        # Dateline/article formats such as "August 20" or "20 August".
        # Only use the current research year; this prevents an undated old
        # reference later in a story from silently becoming a 2026 date.
        current_year = datetime.now(ZoneInfo("Asia/Manila")).year
        m = re.search(rf"\b({months}|{short})\s+(\d{{1,2}})\b", raw, re.I)
        if m:
            mon = month_map.get(m.group(1)[:3].lower())
            try:
                return date(current_year, mon, int(m.group(2)))
            except (ValueError, TypeError):
                pass
        ...
        if m:
            mon = month_map.get(m.group(2)[:3].lower())
            try:
                return date(current_year, mon, int(m.group(1)))
            except (ValueError, TypeError):
                pass
...
def requested_development_count(query, default=5):
    """Extract an explicit requested development count without confusing
    "one source for each development" with "one development".

    Prefer explicit plural "N developments" phrases first.  This is important
    for exact-count research prompts where the same prompt also contains
    language such as "one source for each development".
    """
    ...
    number_pattern = r"([1-9]|10|one|two|three|four|five|six|seven|eight|nine|ten)"

    # First, match the explicit plural target.  The previous implementation
    # used a broad window before "development(s)", so the phrase
    # "one source for each development" could overwrite an earlier
    # "3 developments" match and incorrectly reduce the request to one.
    plural_pattern = rf"\b{number_pattern}\b(?:\s+\w+){{0,5}}\s+developments\b"
    plural_matches = list(re.finditer(plural_pattern, text))
    if plural_matches:
        raw = plural_matches[-1].group(1)
        return int(raw) if raw.isdigit() else word_counts[raw]

    # Singular requests are intentionally narrower: require the count to be
    # immediately associated with "development" (or "single development").
    singular_patterns = (
        rf"\b{number_pattern}\s+development\b",
        r"\ba single development\b",
    )
    for pattern in singular_patterns:
        match = re.search(pattern, text)
        if match:
            raw = match.group(1) if match.lastindex else "one"
            return int(raw) if raw.isdigit() else word_counts[raw]
