...
def source_date(source):
    ...
    # Handle ISO/RFC-style timestamps first.
    try:
    ...
    # Tavily can return a plain YYYY-MM-DD or common written dates.
    patterns = [
        (r"(20\d{2})[-/](\d{1,2})[-/](\d{1,2})", "%Y-%m-%d"),
        (r"(\d{1,2})[ -]([A-Za-z]{3,9})[ -](20\d{2})", "%d %B %Y"),
        (r"([A-Za-z]{3,9})[ ,]+(\d{1,2})[ ,]+(20\d{2})", "%B %d %Y"),
    ]
...
def is_relevant_ai_source(source, today):
    ...
    # For a "today" query, a known older publication date is never treated
    # as current. Missing metadata is allowed because Tavily can omit it.
    if published is not None and published != today:
        return False

    hard_noise = {
        "prediction market", "prediction markets", "release date odds",
        "release date predictions", "kalshi", "polymarket", "coupon",
        "horoscope", "weather", "recipe", "conference registration",
        "webinar", "opinion", "editorial", "commentary", "analysis",
        "forecast", "price target", "stock analysis", "bonus features",
        "roundup", "newsletter", "boardroom imperative",
    }
    if any(term in title_lower for term in hard_noise):
        return False

    # Commentary/"why" pieces are not developments even when they mention
    # a major AI company.
    if title_lower.startswith(("why ", "how ", "what is ", "a guide ")):
        return False

    ai_title_terms = set(RESEARCH_AI_TERMS) | {
        "gpt", "chatgpt", "claude", "llama", "copilot", "artificial intelligence",
        "ai-powered", "ai model", "ai agent", "ai chip", "ai infrastructure",
    }
    major_ai_names = {
        "openai", "anthropic", "google deepmind", "deepmind", "gemini",
        "google ai", "meta ai", "microsoft", "nvidia", "mistral",
        "deepseek", "hugging face", "huggingface", "qwen", "z.ai",
    }
    infra_title_terms = {"chip", "chips", "semiconductor", "data center", "datacenter", "infrastructure", "robotics"}

    ai_in_title = any(term in title_lower for term in ai_title_terms)
    ai_in_title = ai_in_title or any(name in title_lower for name in major_ai_names)
    infra_story = any(term in title_lower for term in infra_title_terms) and any(
        term in content_lower[:2200] for term in RESEARCH_AI_TERMS
    )
    if not ai_in_title and not infra_story:
        return False

    # The headline itself must describe an actual development, not merely
    # a person commenting on AI or an article explaining an existing policy.
    development_terms = set(EVENT_TERMS) | {
        "launch", "launches", "launched", "release", "releases", "released",
        "announce", "announces", "announced", "unveil", "unveils", "unveiled",
        "introduce", "introduces", "introduced", "debut", "rolls out", "rolled out",
        "ships", "shipped", "available", "partnership", "partners", "partnered",
        "develop", "develops", "developed", "developing", "working with",
        "acquire", "acquires", "acquired", "fund", "funds", "funding", "commits",
        "committed", "investment", "invests", "policy", "agreement", "approved",
        "research", "study", "model", "agent", "chip", "infrastructure",
        "opens", "opened", "expands", "expand", "deploys", "deployed",
    }
    if not any(term in title_lower for term in development_terms):
        return False

    # A title containing "says" alone is commentary, not a product/research/
    # policy/partnership development.
    if "says" in title_lower and not any(
        term in title_lower for term in {
            "announces", "announced", "launches", "launched", "released",
            "policy", "approved", "partnership", "acquired", "deploys",
        }
    ):
        return False
...
async def semantic_research_check(source, today):
    """Use the configured AI model to judge whether a research result is a
    genuine, current, substantive AI development. Returns (accepted, score).
    """
    title = clean_text(source.get("title", ""))
    content = clean_text(source.get("content", ""))[:2600]
    published = source_date(source)
    prompt = f"""
You are NEXUS's research-quality gate.

Evaluate ONE news result for the user query: "What are the latest AI developments today?"
...
Accept ONLY if the article itself reports a substantive AI development that
is current/recent enough for today's news request. Examples include a model or
product launch, a major AI research result, a concrete company partnership or
acquisition directly involving AI, an important AI infrastructure development,
or a concrete government/regulatory action involving AI.

Reject:
- CEO/executive comments, interviews, opinions, essays, or commentary when no
  concrete action/development is being reported
- generic analysis or explainers
- roundups/newsletters that bundle many unrelated stories
- prediction-market/odds/forecast pages
- articles where AI is only a minor mention
- old news when the publication date is known to be before today
- duplicate/repackaged commentary about an older event

Return ONLY valid JSON in exactly this form:
{{"accept": true, "score": 0.0, "reason": "brief reason"}}

Score 0.0-1.0 for how strongly this is a genuine AI development worth putting
in a "latest AI developments" answer. Do not accept merely because the title
contains an AI company name.
"""
    try:
        ...
        return bool(verdict.get("accept")) and score >= 0.70, max(0.0, min(score, 1.0))
    except Exception:
        return False, 0.0


async def semantic_filter_sources(sources, today):
    """Classify the strongest candidates concurrently and keep only genuine
    developments. If the classifier is unavailable, preserve the existing
    deterministic filter rather than inventing acceptance.
    """
    candidates = sources[:12]
    checks = await asyncio.gather(
        *(semantic_research_check(source, today) for source in candidates),
        return_exceptions=True,
    )
    accepted = []
    for source, result in zip(candidates, checks):
        if isinstance(result, Exception):
            continue
        ok, score = result
        if ok:
            source["_semantic_score"] = score
...
async def research(query):
    ...
    queries = [
        ...
        f"OpenAI Anthropic Google Gemini Meta AI launch announcement {date_text}",
        f"AI research breakthrough study published {date_text}",
        f"AI agents robotics product deployment launch {date_text}",
        f"AI regulation policy government action announced {date_text}",
        f"AI chips semiconductor infrastructure investment announcement {date_text}",
    ]
    ...
    # Keyword filtering gets candidates; semantic classification decides
    # whether each candidate is actually a substantive current AI development.
    semantic = await semantic_filter_sources(ranked, today)
    semantic.sort(
        key=lambda s: (s.get("_semantic_score", 0.0), research_score(s, today)),
        reverse=True,
    )
    final = select_distinct_sources(semantic, limit=5)
    for source in final:
        source.pop("_semantic_score", None)
