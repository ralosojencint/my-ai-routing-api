...
async def research_synthesis(query, sources):
    """Synthesize only the retrieved source material; never research from memory."""
    if not sources:
        return ""
...
Answer the user's question using ONLY the source evidence below. Do not add facts
from your own knowledge. If a source is weak, unclear, or conflicting, say so.
Group related stories instead of repeating the same event. Keep the answer concise.
For each development, explain what happened and why it matters when the evidence
supports that. Do not invent dates, numbers, people, or claims.
...
async def research_pipeline(query):
    ...
    if sources:
        st.session_state.activity.append(f"Verified {len(sources)} current sources")
        ...
        if not draft:
            ...
            for source in sources:
                ...
                if len(verified) == 5:
                    break
            if verified:
                draft = "\n\n".join(f"{i}. {item}" for i, item in enumerate(verified, 1))
