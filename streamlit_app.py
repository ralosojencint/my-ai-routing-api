import os, re, json, sqlite3, asyncio, time, io, base64, hashlib, uuid, subprocess, sys
    ...
    async def search(self, query, *, today, max_results=10, day_scoped=True):
        ...
        try:
            ...
            try:
                ...
                if day_scoped: fallback["time_range"]="day"
                result=await asyncio.to_thread(self.client.search,**fallback)
                return [x for x in (result.get("results",[]) if isinstance(result,dict) else []) if isinstance(x,dict)]
            except Exception:
                if "forbidden" in text or "403" in text: raise PermissionError("Tavily rejected the request (Forbidden/403).") from exc
                if "429" in text or "rate limit" in text or "quota" in text: raise RuntimeError("Tavily rate limit/quota reached.") from exc
...
class GoogleNewsRSSProvider(ResearchProvider):
    ...
    async def search(self, query, *, today, max_results=10, day_scoped=True):
        rss_query = f"{query} when:1d" if day_scoped else query
        url=self.endpoint+"?"+urlencode({"q":rss_query,"hl":"en-US","gl":"US","ceid":"US:en"})
        def fetch():
            req=Request(url,headers={"User-Agent":"NEXUS-AI/phase3-research","Accept":"application/rss+xml, application/xml;q=0.9, */*;q=0.8"})
        ...
        for item in root.findall('.//item')[:max_results]:
            title=clean_text(item.findtext('title','')); link=clean_text(item.findtext('link','')); desc=clean_text(item.findtext('description','')); pub=clean_text(item.findtext('pubDate',''))
            if title and link:
                results.append({"title":title,"url":link,"content":desc,"published_date":pub,"source_provider":self.name,"publisher":clean_text(item.findtext("source", ""))})
        return results
...
async def research(query):
    ...
    if exact_today or target_date==today:
        queries=[focus,"AI artificial intelligence news developments","AI model launch release announcement","OpenAI Anthropic Google Gemini Meta AI NVIDIA announcement","AI research study breakthrough","AI agents robotics product deployment","AI regulation policy government","AI chips infrastructure technology announcement","generative AI enterprise deployment product"]
    else:
        queries=[f"{focus} {date_text}",f"AI artificial intelligence news developments {date_text}",f"AI model launch release announcement {date_text}",f"OpenAI Anthropic Google Gemini Meta AI NVIDIA announcement {date_text}",f"AI research study breakthrough {date_text}",f"AI agents robotics product deployment {date_text}",f"AI regulation policy government {date_text}",f"AI chips infrastructure technology announcement {date_text}",f"generative AI enterprise deployment product {date_text}"]
    all_sources,provider_errors=await ResearchOrchestrator().search(queries,today=today,day_scoped=bool(exact_today or target_date==today),max_results=10)
    def unique(items):
        out=[]; seen=set()
        for item in items:
    ...
    if len(filtered)<requested_count and (exact_today or target_date==today):
        relaxed_terms=("openai","anthropic","google","gemini","deepmind","meta ai","microsoft","nvidia","mistral","deepseek","hugging face","huggingface","qwen","artificial intelligence","generative ai","machine learning","language model","llm","foundation model","ai agent","ai agents","ai chip","ai chips","robotics","robot")
        ...
        for item in sorted(unique_sources,key=lambda x:research_score(x,today),reverse=True):
            ...
            if published is not None and published not in allowed_dates: continue
            if len(title)<12 or len(content)<40 or not any(t in low for t in relaxed_terms) or any(t in low for t in relaxed_noise): continue
            key=normalize_url(item.get("url",""))
