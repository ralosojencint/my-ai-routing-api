...
def requested_development_count(query, default=5):
    ...
    # BUGFIX (Stress Test 10): a request like "...3 current AI developments...
    # Use exactly one source for each development..." was matching TWICE —
    # once on the genuine "3 current AI developments" and once on the
    # distributive phrase "one source for each development" (which is about
    # sources-per-item, not the total count). Since the old code took the
    # LAST match unconditionally, it silently used "1" instead of "3",
    # collapsing the entire pipeline's target count. Drop any match whose
    # intervening words mark it as a per-item phrase rather than a total
    # count before picking the final match.
    distributive_terms = {"source", "sources", "each", "per", "every", "single"}
    genuine = [
        m for m in matches
        if not (set(re.findall(r"[a-z]+", m.group(2))) & distributive_terms)
    ]

    if genuine:
        raw = genuine[-1].group(1)
...
async def research(query):
    ...
    async def search_one(q,time_range="day"):
        ...
        silently zeroes out an entire day-scoped query."""
    ...
    # Keep the activity trail precise enough to diagnose future verification
    # failures without exposing raw provider payloads to the user.
    st.session_state.activity.append(
        f"Research verification: {len(unique_sources)} unique result(s) → {len(filtered)} eligible AI result(s) → {len(final)} distinct development source(s)"
    )
