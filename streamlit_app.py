...
async def research(query):
    """Phase-2 research: broad retrieval, strict date verification, distinct-event selection."""
    ...
    # Deliberately cover different AI-development categories so one dominant
    # story cannot consume the entire retrieval set.
    queries = [
        f"{focus} {pretty_date}",
        f"AI model launch release announcement {pretty_date}",
        f"OpenAI Anthropic Google Gemini DeepMind Meta AI announcement {pretty_date}",
        f"NVIDIA Mistral DeepSeek Qwen AI model technology announcement {pretty_date}",
        f"AI research study breakthrough published {pretty_date}",
        f"AI agents enterprise deployment product launch {pretty_date}",
        f"humanoid robotics AI robot deployment company announcement {pretty_date}",
        f"AI chips data centers infrastructure announcement {pretty_date}",
        f"AI regulation policy government decision {pretty_date}",
        f"generative AI enterprise product partnership acquisition {pretty_date}",
        f"AI safety science medical research AI announcement {pretty_date}",
    ]

    async def search_one(q, time_range="day", domains=None):
        kwargs = {
            "query": q,
    ...
    # Use several independent outlet groups in the recovery pass. This is not
    # source selection; it only increases the candidate pool.
    batches = await asyncio.gather(
        *(search_one(q, "day" if (exact_today or target_date == today) else None)
          for q in queries)
    )
    all_sources = [x for batch in batches for x in batch]
    ...
    def strict_valid(items):
        out = []
        for item in items:
            if not is_relevant_ai_source(item, today, target_date=target_date):
                continue
            if exact_today or target_date == today:
                if source_date(item) != today:
                    continue
            out.append(item)
        return out
    ...
    # If the first pass is sparse, search by individual high-quality outlets
    # and by concrete event categories. Same-day date verification remains
    # mandatory; these searches only broaden discovery.
    if len(filtered) < requested_count and (exact_today or target_date == today):
        recovery_queries = [
            f"AI news {pretty_date}",
            f"artificial intelligence latest news {pretty_date}",
            f"AI announcement today {pretty_date}",
            f"AI model today {pretty_date}",
            f"AI robotics today {pretty_date}",
            f"AI research today {pretty_date}",
            f"AI business deployment today {pretty_date}",
            f"AI chips infrastructure today {pretty_date}",
        ]
        outlet_groups = [
            ...
            ["counterpointresearch.com", "newsroom.ibm.com"],
        ]
        recovery_batches = await asyncio.gather(
            *(search_one(q, "day") for q in recovery_queries),
            *(search_one(f"{q} {pretty_date}", "day", domains=domains)
              for q in recovery_queries[:5] for domains in outlet_groups)
        )
        all_sources.extend(x for batch in recovery_batches for x in batch)
        candidates = unique(all_sources)
        filtered = strict_valid(candidates)
    ...
    err = "" if len(final) >= requested_count else (
        f"Only {len(final)} independently verified current AI development(s) "
        f"were available; {requested_count} requested."
    )
    return {
        "sources": final,
...
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        ...
        if message.get("sources") and "### Sources" not in str(message.get("content", "")):
...
if chat_value:
    ...
    with st.chat_message("assistant"):
        ...
        if result["sources"] and "### Sources" not in str(result.get("answer", "")):
            with st.expander("Sources"):
