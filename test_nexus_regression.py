...
def load_functions():
    ...
    ns = {
        're': re, 'st': type('ST', (), {'session_state': type('SS', (), {'datasets': [], 'documents': []})()})(),
        'date': date, 'datetime': datetime, 'parsedate_to_datetime': parsedate_to_datetime,
        'urlsplit': urlsplit, 'urlunsplit': urlunsplit, 'parse_qsl': parse_qsl, 'urlencode': urlencode,
    }
...
def test_url_and_event_deduplication():
    ns = load_functions()
    assert ns['normalize_url']('https://www.example.com/story/amp?utm_source=x&x=1') == ns['normalize_url']('https://example.com/story?x=1')
    a = source('Apple launches Mac Studio with new AI chips', 'https://example.com/a', 'Apple launched Mac Studio featuring new chips designed for AI workloads.', '2026-09-03')
    b = source('Apple announces Mac Studio with AI-focused chips', 'https://example.net/b', 'Apple announced Mac Studio with chips focused on AI workloads.', '2026-09-03')
    c = source('Anthropic launches a new enterprise AI model', 'https://example.org/c', 'Anthropic launched a new enterprise AI model for business customers.', '2026-09-03')
...
def test_relevance_and_noise_gates():
    ns = load_functions()
    good = source('Apple launches Mac Studio with new AI chips', 'https://example.com/a', 'Apple launched Mac Studio featuring new chips designed for AI workloads.', '2026-09-03')
    old = dict(good, published_date='2026-09-02')
    noise = source('AI activists protest OpenAI data center expansion', 'https://example.com/p', 'Activists protested an AI infrastructure project.', '2026-09-03')
    assert ns['is_relevant_ai_source'](good, date(2026, 9, 3))
    assert not ns['is_relevant_ai_source'](old, date(2026, 9, 3), target_date=date(2026, 9, 3))
...
def test_output_contract_and_rejections():
    ...
    assert not ns['validate_research_output'](fallback.replace('2. **', '1. **', 1), QUERY, selected)
    assert not ns['validate_research_output'](fallback.replace('[Source 2]', '[Source 9]'), QUERY, selected)
    assert not ns['validate_research_output'](fallback + '\n3. Extra — https://example.com/extra', QUERY, selected)
    assert not ns['validate_research_output'](fallback + '\n\n### Sources\n1. Extra — https://example.com/extra', QUERY, selected)
    missing = re.sub(r'^- Why it matters:.*$', '', fallback, flags=re.M)
    assert not ns['validate_research_output'](missing, QUERY, selected)
    ...
    assert ns['validate_research_output'](insufficient, QUERY, [c])


def test_router_contracts():
    ns = load_functions()
    assert str(ns['detect_route']('Find the latest AI developments today')).lower() != 'general'
    assert 'forex' in str(ns['detect_route']('Show me high-impact USD events on Forex Factory')).lower()
