...
async def gemini_text(prompt, images=None):
    ...
    if images:
        prompt += "\n\nVISUAL ACCURACY: Describe only what is supported by the image. Never invent a character name, anime, franchise, location, or identity. If identity is uncertain, say so and describe visible features instead.\n"
...
REPUTABLE_NEWS_DOMAINS = {"reuters.com", "apnews.com", "bbc.com", "nytimes.com", "theguardian.com", "wsj.com", "bloomberg.com", "cnbc.com", "techcrunch.com", "arstechnica.com", "theverge.com", "washingtonpost.com", "cnn.com"}
WEAK_RESEARCH_DOMAINS = {"linkedin.com", "trendhunter.com", "investing.com", "stocktitan.net", "intellectia.ai", "biggo.com", "ua.news"}
...
def research_score(source, today):
    ...
    if domain in PRIMARY_DOMAINS:
        ...
        score += 8
    elif domain in WEAK_RESEARCH_DOMAINS:
        score -= 10
...
async def research(query):
    ...
    for source in unique:
        if not is_relevant_ai_source(source):
            continue
        published = source_date(source)
...
async def answer_user(query, images=None):
    ...
    for d in st.session_state.datasets:
        ...
        if not numeric.empty:
            lines.append("Deterministic numeric summaries:")
            for col in numeric.columns:
                lines.append(
                    f"- {col}: sum={numeric[col].sum()}, mean={numeric[col].mean()}"
                )
...
Do not reveal private chain-of-thought.
Never invent facts that are not supported by the supplied context.
...
- Never present an uncertain character/person identity as fact.
- If the user did not provide a name and the image does not prove identity, do not name the character.
- Prioritize appearance, clothing, objects, pose, expression, colors, and background.
"""
