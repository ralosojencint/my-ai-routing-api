...
div[data-testid="stHorizontalBlock"] > div { width: auto !important; padding: 0 !important; margin: 0 !important; }
div[data-testid="stHorizontalBlock"] > div:nth-child(2) { flex-grow: 2 !important; width: 100% !important; }
...
if not st.session_state["is_premium"] and st.session_state["anonymous_clicks"] >= 3:
    st.error("🛑 Daily Session Limit Reached")
else:
    ...
    if st.session_state["text_out"]:
        ...
        try:
            ...
            pdf_holder.download_button(label="📥 Download Response as PDF Document", data=bytes(pdf.output()), file_name="nexus_report.pdf", mime="application/pdf")
        except Exception: pass

    # ========================================================================================
    # 📱 THE COMPACT INTERFACE ROW (No rigid st.form hooks to cause vertical mobile breakage)
    # ========================================================================================
    # Explicitly weights layout segments side-by-side to guarantee clean mobile horizontal rendering
    bar_cols = st.columns([1, 6, 1])
    ...
    if execute_btn and (user_input or uploaded_image):
