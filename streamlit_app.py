...
def is_relevant_ai_source(source, today, target_date=None, day_scoped=False):
...
async def research(query):
    ...
    def valid(items):
        ...
        for x in items:
            ...
            if not is_relevant_ai_source(x,today,target_date=target_date,day_scoped=day_scoped):
                continue
            published=source_date(x)
            # Tavily's time_range=day is already a freshness constraint. Some
            # providers/results omit publication metadata, so a missing date
            # must NOT be treated as proof that the result is stale. When an
            # explicit date is present, still enforce the requested date.
            if exact_today or target_date==today:
                if published is not None and published != today:
                    continue
            elif target_date is not None and published is not None:
                if published != target_date:
                    continue
            out.append(x)
        return out
    ...
    def relaxed_filter(pool):
        ...
        for source in pool:
            ...
            # date that is outside today's window.
            if published is not None and published != today:
                continue
