...
async def research(query):
    ...
    target_date = requested_research_date(query, today)
    if target_date is None and requires_exact_today(query):
        target_date = today
    date_text = (target_date or today).isoformat()
    requested_count = requested_development_count(query, default=5)
    ...
    queries = [
        f"{focus} AI developments {date_text}",
    ...
    filtered = [s for s in unique if is_relevant_ai_source(s, today, target_date=target_date)]

    # If the user explicitly asked for evidence published today, every selected
    # source must have a verifiable publication date. Do not silently treat
    # missing metadata as today's evidence in that case.
    if requires_exact_today(query):
        filtered = [s for s in filtered if source_date(s) == today]
...
async def research_pipeline(query):
    """Live AI-news retrieval + grounded synthesis behind the Phase-2 router."""
    ...
    if sources:
        st.session_state.activity.append(
            f"Selected {len(sources)} source(s) for the requested research output"
        )

        # Prefer an AI synthesis grounded strictly in the retrieved evidence.
        draft = await research_synthesis(query, sources)
        if draft:
            st.session_state.activity.append("Research synthesis completed")

        # Safe deterministic fallback: the app still returns the verified
        # source material if synthesis is unavailable.
        if not draft:
            verified = []
            for i, source in enumerate(sources[:requested_development_count(query, default=5)], 1):
                summary = source_grounded_summary(source)
                if summary:
                    verified.append(f"{i}. {summary} [Source {i}]")
            if verified:
                draft = "\n\n".join(verified)
