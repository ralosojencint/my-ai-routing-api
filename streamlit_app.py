...
async def research_pipeline(query):
    ...
    if exact_contract:
        ...
        st.session_state.activity.append(f"Synthesis contract check: {'passed' if synthesis_contract_ok else 'failed'}")
        if synthesis_contract_ok:
            synthesis_grounding_ok = validate_source_grounding(draft, sources, query)
            st.session_state.activity.append(f"Synthesis grounding check: {'passed' if synthesis_grounding_ok else 'failed'}")
        ...
        if sources and n <= len(sources):
            ...
            st.session_state.activity.append(f"Fallback contract check: {'passed' if fallback_contract_ok else 'failed'}")
            if fallback_contract_ok:
                fallback_grounding_ok = validate_source_grounding(fallback, sources, query)
                st.session_state.activity.append(f"Fallback grounding check: {'passed' if fallback_grounding_ok else 'failed'}")
