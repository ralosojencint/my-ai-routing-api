...
if not st.session_state["is_premium"] and st.session_state["anonymous_clicks"] >= FREE_DAILY_LIMIT:
    ...
    if st.session_state["text_out"]:
        ...
        try:
            ...
            pdf_holder.download_button(label="📥 Download Response as PDF", data=bytes(pdf_bytes), file_name="nexus_report.pdf", mime="application/pdf")
        ...
        if not user_input and not uploaded_image and not audio_file:
            ...
            if generate_art_mode:
                ...
                if url:
                    ...
                    except Exception as e: st.session_state["text_out"] = f"❌ Scraper Error: {str(e)}"
