...
def validate_research_output(draft, query, sources):
    ...
    if verified_count:
        ...
        for index in range(1, verified_count + 1):
            ...
            if not block_sources:
                return False
            # Normally each development is source-isolated. If multiple sources
            # appear, require an explicit conflict acknowledgement so the model
            # cannot silently merge contradictory evidence.
            conflict_ack = re.search(
                r"\b(conflict|conflicting|disagree|disagreement|contradict|contradiction|different accounts|sources differ)\b",
                block,
            )
            if block_sources != {index} and not conflict_ack:
                return False
    ...
    cited_numbers=[]
    for match in re.finditer(r"(?ms)^\s*(?:[1-9]|10)\.\s+(.*?)(?=^\s*(?:[1-9]|10)\.\s+|\Z)", text.split("### Sources",1)[0]):
