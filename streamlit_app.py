...
# Visual CSS styling to upgrade the interface UI and force elements onto a true single horizontal capsule bar
st.markdown("""
<style>
...
/* HARD-FORCING EVERY CONTROL ONTO 1 SINGLE HORIZONTAL PILL CONTAINER ROW AT THE BOTTOM */
...
/* DESIGN UPGRADE: Turning the send button into a premium vibrant blue circle button matching ChatGPT */
.send-btn-box button {
...
.send-btn-box button:hover { background-color: #1d4ed8 !important; }
</style>
...
if not st.session_state["is_premium"] and st.session_state["anonymous_clicks"] >= 3:
    ...
    # =========================================================================================
    # 📱 THE COMPACT HORIZONTAL PILL BAR DOCK (Bypasses vertical framework block breakage)
    # ========================================================================================
    ...
    with pill_cols[2]:
        ...
        # Injected the requested clean square stop glyph inside the native trigger loop sequence
        ...
        if not user_input and not u_valid:
            ...
            if art_valid and user_input:
                ...
                try:
                    ...
                    # Saving answer data directly inside global cache states to freeze content output safely
