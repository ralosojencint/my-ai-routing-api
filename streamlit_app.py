    ...
    if should_research(query):
        ...
        if sources:
            ...
            if is_forex_query(query):
                ...
                if verified:
                    draft = "**Forex Factory — High Impact / Economic Calendar**\n\n" + "\n\n".join(
                        f"{i}. {item}" for i, item in enumerate(verified, 1)
                    )
                else:
                    draft = (
                        "⚠️ NEXUS found Forex Factory results, but could not extract "
                        "the calendar details from them."
                    )

            elif asks_latest_model:
                model_sources = [
                    s for s in sources
        ...
        if not draft:
            if is_forex_query(query):
                draft = (
                    "⚠️ NEXUS could not find usable Forex Factory economic-calendar "
